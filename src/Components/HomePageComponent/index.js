...
class HomePageComponent extends Component {
  state = {
    ...
    score: 0,
    seconds: 60,
    bannerImageUrl: imagesList[0].imageUrl,
    gameOverPageView: false,
  }
  ...
  thumbnailTriggered = idNum => {
    const {bannerImageUrl} = this.state
    const indexNum = Math.floor(Math.random() * imagesList.length)
    const newImageUrl = imagesList[indexNum].imageUrl
    // console.log(idNum)
    // console.log(indexNum)
    const findImageUrlArrayOfObject = imagesList.filter(
      eachObject => eachObject.id === idNum,
    )
    const foundImageUrl = findImageUrlArrayOfObject[0].imageUrl
    if (foundImageUrl === bannerImageUrl) {
      this.setState(prevState => ({
        bannerImageUrl: newImageUrl,
        score: prevState.score + 1,
      }))
    } else {
      this.setState({gameOverPageView: true})
    }
  }
      ...
      if (gameOverPageView || seconds === 0) {
        ...
        return (
          <div className="gameOverContainer">
            ...
            <button
              type="button"
              className="btnPlayStyle"
              onClick={this.playAgainBtnClicked}
            >
              <img
          ...
          <ul className="ulTabItems">
            {initialItems.map(eachObjectItem => (
              <AppItem
                ...
                thumbnailTriggeredProp={this.thumbnailTriggered}
